...
void adc_init(ADC_ch adc_ch,ADC_CMD cmd)
{
    ...
    ADC_RES=0;
    ADC_RESL=0;
    ADC_CONTR = ADC_POWER|cmd|adc_ch ;
...
void pwm_duty(PCA_n pca,uint8 duty)
{
    switch(pca)
    {
        case PCA_0: CCAP0H = CCAP0L = duty;
        break;
        case PCA_1: CCAP1H = CCAP1L = duty;
        break;
    }
}
...
void pwm_perc(PCA_n pca,uint8 perc)
{
   ...
   switch(pca)
    {
        case PCA_0: CCAP0H = CCAP0L = (uint8)(perc*2.56);
        break;
        case PCA_1: CCAP1H = CCAP1L = (uint8)(perc*2.56);
        break;
    } 
}
